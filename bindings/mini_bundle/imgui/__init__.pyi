...
from mini_bundle.imgui.internal import Context, ImDrawListSharedData, ImFontBuilderIO
ImDrawVert = Any
...
#    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX function
#    returned True. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
# IMGUI_API bool          BeginChild(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags window_flags = 0);    /* original C++ signature */
@overload
def begin_child(
    str_id: str,
...
# IMGUI_API bool          BeginChild(ImGuiID id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags window_flags = 0);    /* original C++ signature */
@overload
def begin_child(
    id_: ID,
    size: ImVec2 = ImVec2(0, 0),
...
# IMGUI_API void          PushTabStop(bool tab_stop);                                         /* original C++ signature */
def push_tab_stop(tab_stop: bool) -> None:
    """== tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default but you can disable it for certain widgets"""
    pass

# IMGUI_API void          PopTabStop();    /* original C++ signature */
...
# - Note that ImageButton() adds style.FramePadding*2.0 to provided size. This is in order to facilitate fitting an image in a button.
...
def columns(count: int = 1, id_: Optional[str] = None, border: bool = True) -> None:
    pass
...
def begin_child_frame(id_: ID, size: ImVec2, flags: WindowFlags = 0) -> bool:
...
class WindowFlags_(enum.Enum):
    ...
    nav_flattened = (
        enum.auto()
    )  # (= 1 << 23)  # [BETA] On child window: allow gamepad/keyboard navigation to cross over parent border to this child or between sibling child windows.
...
class InputTextFlags_(enum.Enum):
    ...
    chars_uppercase = enum.auto()  # (= 1 << 2)  # Turn a..z into A..Z
    ...
    callback_edit = (
        enum.auto()
    )  # (= 1 << 19)  # Callback on any edit (note that InputText() already returns True on edit, the callback is useful mainly to manipulate the underlying buffer while focus is active)
    # ImGuiInputTextFlags_EscapeClearsAll     = 1 << 20,      /* original C++ signature */
    escape_clears_all = (
        enum.auto()
    )  # (= 1 << 20)  # Escape key clears content if not empty, and deactivate otherwise (contrast to default behavior of Escape to revert)
...
class TreeNodeFlags_(enum.Enum):
    ...
    open_on_double_click = enum.auto()  # (= 1 << 6)  # Need double-click to open node
    ...
    frame_padding = (
        enum.auto()
    )  # (= 1 << 10)  # Use FramePadding (even for an unframed text node) to vertically align text baseline to regular widget height. Equivalent to calling AlignTextToFramePadding().
    ...
    # ImGuiTreeNodeFlags_SpanAllColumns       = 1 << 13,      /* original C++ signature */
    span_all_columns = (
        enum.auto()
    )  # (= 1 << 13)  # Frame will span all columns of its container table (text will still fit in current column)
    # ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 14,      /* original C++ signature */
...
class TabBarFlags_(enum.Enum):
    ...
    no_close_with_middle_mouse_button = (
        enum.auto()
    )  # (= 1 << 3)  # Disable behavior of closing tabs (that are submitted with p_open != None) with middle mouse button. You can still repro this behavior on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = False.
...
class TabItemFlags_(enum.Enum):
    ...
    no_close_with_middle_mouse_button = (
        enum.auto()
    )  # (= 1 << 2)  # Disable behavior of closing tabs (that are submitted with p_open != None) with middle mouse button. You can still repro this behavior on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = False.
...
class HoveredFlags_(enum.Enum):
    ...
    no_nav_override = (
        enum.auto()
    )  # (= 1 << 11)  # IsItemHovered() only: Disable using gamepad/keyboard navigation state when active, always query mouse
...
class SortDirection_(enum.Enum):
    """A sorting direction"""
...
class Key(enum.Enum):
    ...
    im_gui_mod_ctrl = enum.auto()  # (= 1 << 12)  # Ctrl
...
class ConfigFlags_(enum.Enum):
    ...
    no_mouse = (
        enum.auto()
    )  # (= 1 << 4)  # Instruct imgui to clear mouse position/buttons in NewFrame(). This allows ignoring the mouse information set by the backend.
    ...
    docking_enable = enum.auto()  # (= 1 << 6)  # Docking enable flags.
...
class Col_(enum.Enum):
    ...
    docking_empty_bg = (
        enum.auto()
    )  # (= 39)  # Background color for empty node (e.g. CentralNode with no window docked into it)
    # ImGuiCol_PlotLines,    /* original C++ signature */
    plot_lines = enum.auto()  # (= 40)
    # ImGuiCol_PlotLinesHovered,    /* original C++ signature */
    plot_lines_hovered = enum.auto()  # (= 41)
    # ImGuiCol_PlotHistogram,    /* original C++ signature */
    plot_histogram = enum.auto()  # (= 42)
    # ImGuiCol_PlotHistogramHovered,    /* original C++ signature */
    plot_histogram_hovered = enum.auto()  # (= 43)
    ...
    table_row_bg_alt = enum.auto()  # (= 48)  # Table row background (odd rows)
    # ImGuiCol_TextSelectedBg,    /* original C++ signature */
    text_selected_bg = enum.auto()  # (= 49)
    # ImGuiCol_DragDropTarget,            /* original C++ signature */
    drag_drop_target = enum.auto()  # (= 50)  # Rectangle highlighting a drop target
    ...
    nav_windowing_dim_bg = (
        enum.auto()
    )  # (= 53)  # Darken/colorize entire screen behind the CTRL+TAB window list, when active
    # ImGuiCol_ModalWindowDimBg,          /* original C++ signature */
    modal_window_dim_bg = (
        enum.auto()
    )  # (= 54)  # Darken/colorize entire screen behind a modal window, when one is active
...
class StyleVar_(enum.Enum):
    ...
    tab_rounding = enum.auto()  # (= 22)  # float     TabRounding
    # ImGuiStyleVar_TabBarBorderSize,        /* original C++ signature */
    tab_bar_border_size = enum.auto()  # (= 23)  # float     TabBarBorderSize
    # ImGuiStyleVar_ButtonTextAlign,         /* original C++ signature */
    button_text_align = enum.auto()  # (= 24)  # ImVec2    ButtonTextAlign
    # ImGuiStyleVar_SelectableTextAlign,     /* original C++ signature */
    selectable_text_align = enum.auto()  # (= 25)  # ImVec2    SelectableTextAlign
    # ImGuiStyleVar_SeparatorTextBorderSize,    /* original C++ signature */
    separator_text_border_size = enum.auto()  # (= 26)  # float  SeparatorTextBorderSize
    # ImGuiStyleVar_SeparatorTextAlign,      /* original C++ signature */
    separator_text_align = enum.auto()  # (= 27)  # ImVec2    SeparatorTextAlign
...
class Style:
    ...
    cell_padding: ImVec2  # Padding within a table cell. CellPadding.y may be altered between different rows.
    ...
    display_window_padding: ImVec2  # Window position are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows.
    # ImVec2      DisplaySafeAreaPadding;    /* original C++ signature */
    display_safe_area_padding: ImVec2  # If you cannot see the edges of your screen (e.g. on a TV) increase the safe area padding. Apply to popups/tooltips as well regular windows. NB: Prefer configuring your TV sets correctly!
...
class IO:
    ...
    platform_locale_decimal_point: ImWchar  # '.'              // [Experimental] Configure decimal point e.g. '.' or ',' useful for some languages (e.g. German), generally pulled from *localeconv()->decimal_point
...
class ImDrawCmd:
    ...
    user_callback_data: Any  # 4-8  // The draw callback code can access this.
...
class ImDrawList:
    ...
    def add_callback(self, callback: ImDrawCallback, callback_data: Any) -> None:
...
class ImFontConfig:
    ...
    rasterizer_multiply: float  # 1.0     // Brighten (>1.0) or darken (<1.0) font output. Brightening small fonts may be a good workaround to make them more readable.
...
class ImFontGlyphRangesBuilder:
    ...
    def build_ranges(self, out_ranges: List[ImWchar]) -> None:
        """Output new ranges"""
        pass
...
class ImFontAtlasCustomRect:
    ...
    glyph_id: int  # Input    // For custom font glyphs only (ID < 0x110000)
...
class ViewportFlags_(enum.Enum):
    ...
    owned_by_app = (
        enum.auto()
    )  # (= 1 << 2)  # Platform Window: Was created/managed by the user application? (rather than our backend)
...
class Viewport:
    ...
    platform_handle: Any  # None* for FindViewportByPlatformHandle(). (e.g. suggested to use natural platform handle such as HWND, GLFWWindow*, SDL_Window*)
